...
import { Produto, getAllProduto, getOneProduto, addProduto, deleteProduto, updateProduto } from "../models/bauduccoModels";
...
export function getOneProdutoController(req: Request, res: Response){
    const { id } = req.params;
    const produtoId = parseInt(id,10);
    const Produtos = getOneProduto(produtoId);
    res.json(Produtos);
}
